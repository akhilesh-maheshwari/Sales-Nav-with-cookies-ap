...
try {
  ...
  console.log('URL       :', salesNavigatorUrl.slice(0, 80) + '...');

  // ── Validate inputs ──
  if (!serviceTagName.trim())    throw new Error('fileName is required!');
  if (!salesNavigatorUrl.trim()) throw new Error('salesNavigatorUrl is required!');
  if (!linkedinCookie.trim())    throw new Error('linkedinCookie is required!');
  if (!leadCount || leadCount < 1) throw new Error('leadCount must be at least 1!');

  // Validate it's actually a Sales Navigator URL
  if (
    !salesNavigatorUrl.includes('linkedin.com/sales/search/') &&
    !salesNavigatorUrl.includes('linkedin.com/sales/lists/')
  ) {
    throw new Error('Please provide a valid LinkedIn Sales Navigator search URL!');
  }
  ...
  while (!batchJobs || batchJobs.length === 0) {
    console.log('⏳ No slots available. Waiting 2 mins before retry...');
  ...
  while (batchJobs && batchJobs.length > 0) {
    ...
    for (const result of batchStatusResults) {
      ...
      if (result.status !== 'Completed') {
        console.log(`  ⚠️ Batch ${batch_number} did not complete. Skipping.`);
        batchResults.push({
          batch_number,
          request_id,
      ...
      // Charge for completed batch
      const batchSize  = job.batch_size || 0;
      const batchCost  = parseFloat((batchSize * 0.005).toFixed(3));
      totalCharged    += batchCost;
      console.log(`  💳 Batch ${batch_number} — Charging for ${batchSize} leads ($${batchCost}). Total: $${totalCharged.toFixed(3)}`);
      await Actor.charge({ eventName: serviceOption1, count: batchSize });

      const boomerangOutputUrl = `https://s1.boomerangserver.co.in/webhook/sales-navigator-scraper-output?request_id=${request_id}`;
      ...
      try {
        ...
        console.log(`  ❌ Output fetch error: ${fetchErr.message}`);
      }
      ...
      batchResults.push({
        ...
        leads_found    : result.profiles_found     || 0,
        leads_not_found: result.profiles_not_found || 0,
